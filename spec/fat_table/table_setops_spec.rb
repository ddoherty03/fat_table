...
module FatTable
  RSpec.describe Table do
    ...
    let(:tab2) do
      ...
      Table.from_aoh(aoh2, hlines: true)
    end
    let(:tab3) do
    ...
    describe 'union' do
      it 'is able to union with a compatible table' do
        utab = tab1.union(tab2)
        expect(utab.rows.size).to eq(6)
        expect(utab.groups.size).to eq(1)
      end

      it 'is able to union_all with a compatible table' do
        utab = tab1.union_all(tab2)
        expect(utab.rows.size).to eq(9)
        expect(utab.groups.size).to eq(3)
      end

      it 'throws an exception for union with different sized tables' do
        expect {
          tab1.union(tab3)
        }.to raise_error(/different number of columns/)
      end

      it 'throws an exception for union with different types' do
        expect {
          tab1.union(tab4)
        }.to raise_error(/different column types/)
      end

      it 'throws exception for union_all with different sized tables' do
        expect {
          tab1.union_all(tab3)
        }.to raise_error(/different number of columns/)
      end

      it 'throws an exception for union_all with different types' do
        expect {
          tab1.union_all(tab4)
        }.to raise_error(/different column types/)
      end
    ...
    describe 'intersect' do
      it 'returns the intersect of two tables' do
        itab = tab1.intersect(tab2)
        expect(itab.size).to eq(1)
        expect(itab.groups.size).to eq(1)
      end

      it 'returns the intersect_all of two tables' do
        # Notice that the order of operands does not matter.
    ...
    describe 'except' do
      it 'returns the except of two tables' do
        itab = tab1.except(tab2)
        expect(itab.size).to eq(2)
        expect(itab.groups.size).to eq(1)
      end

      it 'returns the except_all of two tables' do
      ...
      it 'throws exception for except with different sized tables' do
        expect {
          tab1.except(tab3)
        }.to raise_error(/different number of columns/)
      end

      it 'throws an exception for except with different types' do
        expect {
          tab1.except(tab4)
        }.to raise_error(/different column types/)
      end

      it 'throws exception for except_all with different sized tables' do
        expect {
          tab1.except_all(tab3)
        }.to raise_error(/different number of columns/)
      end

      it 'throws an exception for except_all with different types' do
        expect {
