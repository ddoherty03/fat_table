  ...
  RSpec.describe 'Formatter::AoaFormatter' do
    describe 'table output' do
      ...
      it 'uses 4 decimal places in default format' do
        ...
        expect(nums).to include('795546.2000')
        expect(nums).to include('118186.4000')
    ...
    describe 'Module level formatter' do
      ...
      it 'formats with FatTable.to_aoa' do
        tab1 = tab.select(
          :ref,
          :price,
          :shares,
        ...
        tab_aoa = FatTable.to_aoa(tab1) do |f|
          f.format(price: '0.4', shares: '0.0,', cost: '0.2,', cumulative: '0.2,')
        end
        expect(tab_aoa).to be_an(Array)
        expect(tab_aoa.first).to be_an(Array)
        expect(tab_aoa.first.first).to be_a(String)
      end
    end
  end
end
