...
module FatTable
  RSpec.describe 'Formatter::TextFormatter' do
    ...
    describe 'tolerant table output' do
      let(:tab) do
        aoa = [
          %w[Ref Date Code Raw Shares Price Info Bool],
          [1,    '2013-05-02', 'P', 795_546.20, 795_546.2, 1.1850,  'ZMPEF1', 'Perhaps'],
          [2,    '2013-05-02', 'P', 118_186.40, 118_186.4, 11.8500, 'ZMPEF1', 'T'],
          [5,    '2013-05-02', 'P', 118_186.40, 118_186.4, 11.8500, 'ZMPEF1', 'Maybe'],
          [7,    '2013-05-20', 'S', Date.today, 5046.00,   28.2804, 'ZMEAC',  'F'],
          [8,    '2013-05-20', 'S', 85_000.00,  35_742.50, 28.3224, 'ZMEAC',  'T'],
          [9,    'Paper Moon', 'S', 33_302.00,  14_003.49, 28.6383, 'ZMEAC',  'T'],
          [10,   '2013-05-23', 'S', 8000.00,    3364.00,   27.1083, 'ZMEAC',  'T'],
          [11,   '2013-05-23', 'S', 23_054.00,  9694.21,   26.8015, 'ZMEAC',  'F'],
          [12,   '2013-05-23', 'S', 39_906.00,  16_780.47, 25.1749, 'ZMEAC',  'T'],
          ['13', '2013-05-29', 'S', 13_459.00,  5659.51,   24.7464, 'ZMEAC',  'T'],
      ...
      it 'formats with FatTable.to_text' do
        tab1 = tab.select(
          :ref,
          :price,
          :shares,
        ...
        tab_text = FatTable.to_text(tab1) do |f|
          f.format(price: '0.4', shares: '0.0,', cost: '0.2,', cumulative: '0.2,')
        end
        expect(tab_text).to be_an(String)
        expect(tab_text).to match(/\+[=+]+\+/)
      end
    end
  end
end
