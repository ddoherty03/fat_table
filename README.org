...
For a numeric, all the instructions valid for string are available, in addition
to the following:
...
- d[fmt] :: apply the format to a datetime that has no or a zero hour, minute,
     second components, where fmt is a valid format string for Date#strftime,
