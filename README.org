...
#+BEGIN_SRC ruby
  FatTable::AoaFormatter.new(tab_a) do |f|
    f.format(numeric: '0.0,R', id: '6.0C')
  end.output
#+END_SRC
...
|      Id | Name  | Age | Address    | Salary |  Join Date |
|---------+-------+-----+------------+--------+------------|
| 000,001 | Paul  |  32 | California | 20,000 | 2001-07-13 |
| 000,003 | Teddy |  23 | Norway     | 20,000 | 2007-12-13 |
| 000,004 | Mark  |  25 | Rich-Mond  | 65,000 | 2007-12-13 |
| 000,005 | David |  27 | Texas      | 85,000 | 2007-12-13 |
| 000,002 | Allen |  25 | Texas      |        | 2005-07-13 |
| 000,008 | Paul  |  24 | Houston    | 20,000 | 2005-07-13 |
| 000,009 | James |  44 | Norway     |  5,000 | 2005-07-13 |
| 000,010 | James |  45 | Texas      |  5,000 |            |
#+END_EXAMPLE
...
You can supply a block to these methods as well, and, again, the ~.output~
method is called automatically after the block:
...
And you can supply a block to it as well:
...
To set formatting directives for all locations in a table at once, you use the
~format~ method; to set formatting directives for a particular location in the
table, you use the ~format_for~ method, giving the location as the first
parameter. Other than that first parameter, the two methods take the same types
of parameters.

The remaining parameters are hash-like parameters that use either a column name
or a type as the key and a string with the formatting directives to apply as the
value. For example, this says to set the formatting for all locations in the
table such that all numeric fields are rounded to whole numbers (the '0.0'
part), are right-aligned (the 'R' part), and have grouping commas inserted (the
',' part). But the ~:id~ column is numeric and the second parameter overrides
the formatting for numerics in general and calls for the ~:id~ column to be
padded to three digits with zeros on the left (the '3.0' part) and to be
centered.
