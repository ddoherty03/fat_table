...
Note that the order of the columns is the same as the order of the parameters to
the ~select~ method.  You can re-order the columns with a second, chained call
to ~select~:
